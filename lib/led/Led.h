...
public:
  ...
  // Défini le comportement de la LED afin qu'elle soit contrôlé automatiquement.
  void set_automatic(bool automatic);

  // Défini la luminosité de la LED (valeur entre 0 et 1).
  //
  // Si la LED est contrôlée automatiquement, ce comportement se désactive alors.
  void set_luminosity(double value);

  // Retourne la luminosité de la LED.
  double get_luminosity();

  // S'exécute au niveau de la loupe afin de s'assurer d'avoir le comportement attendu pour la Led.
  void loop();
};
